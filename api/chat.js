...
// ═══════════════════════════════════════════
async function runCOOAI(userText, messages, serperKey, apiKey) {
  const langCode = detectLang(userText);
  const L = COO_LABELS[langCode] || COO_LABELS.en;
  const today = new Date().toLocaleDateString('mk-MK', { day: '2-digit', month: '2-digit', year: 'numeric' });
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ═══ COO AI — MULTI-AGENT ═══
    if (avatar === 'cooai') {
      console.log('[cooai] Activating multi-agent mode');
      const cooReport = await runCOOAI(userText, messages, serperKey, apiKey);
